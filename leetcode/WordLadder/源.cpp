...
class Solution {
public:
    int ladderLengthTLE2(string start, string end, unordered_set<string> &dict) {
        /*unordered_map<string, unordered_set<string>> ndict;
        ...
        int qsize(0);
        string currentword("");
        unordered_set<string> lastlevel = {start};
        while (!q.empty())
        {
            ++level;
            qsize = q.size();
            ...
            {
                currentword = q.front();
                //for (auto t : ndict[currentword])
                for (auto t:dict)
                {
                    if (!ed1(t, currentword))
                        continue;
                    ...
                    if (t != start && lastlevel.find(t) == lastlevel.end())
                    {
                        q.push(t);
                        lastlevel.insert(t);
                    }
    ...
    int ladderLengthTLE1(string start, string end, unordered_set<string> &dict) {
        unordered_set<string> res;
        for (auto d : dict)
        {
            if (ed1(d, end))
    ...
    bool ed1(const string &left, const string &right)
    {
        int ed = 0;
        for (auto i = 0; i < left.size(); ++i)
        {
            ed += (left.at(i) == right.at(i)) ? 0 : 1;
        }
        return ed == 1;
    }

    int ladderLengthAC(string start, string end, unordered_set<string> &dict)
    {
        queue<string> q;
        unordered_set<string> exists;
        int level(1), current(0), levelsize(0);
        string currentword("");
        dict.insert(end);
        q.push(start);
        while (!q.empty())
        {
            ...
            while (current < levelsize)
            {
                ...
                for (auto i = 0; i < currentword.size(); ++i)
                {
                    for (auto j = 1; j < 26; ++j)
                    {
                        currentword.at(i) = (currentword.at(i) + 1 - 'a') % 26 + 'a';
                        //cout << currentword << endl;
                        if (dict.find(currentword) != dict.end() && exists.find(currentword) == exists.end())
                        {
                            q.push(currentword);
                            exists.insert(currentword);
                        }
                    }
                    currentword.at(i) = (currentword.at(i) + 1 - 'a') % 26 + 'a';
                }
    ...
    int ladderLengthMLE(string start, string end, unordered_set<string> &dict)
    {
        queue<string> q;
        int level(1), current(0), levelsize(0);
        string currentword("");
...
int main()
{
    ...
    string start("charge"), end("comedo");
